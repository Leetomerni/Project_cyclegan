function [out1,out2]=Parameter_estimation_IRT( IIraw,param)
for i=1:9
...
for angle_num=1:3
    ...
    pop_temp=Spectrum(:,:,3).*NotchFilter;
    [pop_y,pop_x] = find(pop_temp==max(max(pop_temp)));   
    Dcr=IIraw(:,:,(angle_num-1)*3+1)+IIraw(:,:,(angle_num-1)*...
        3+2).*exp(-1i*(2*pi/3))+IIraw(:,:,(angle_num-1)*...
        3+3).*exp(1i*(2*pi/3));
    Dck=fftshift(fft2(Dcr));
    param.phaOff=phase(Dck(pop_y,pop_x));
    phase_irt=phase(Dck(pop_y,pop_x));
    param.Dir(angle_num).px = kx;                                              % new peak kx
    param.Dir(angle_num).py = ky;                                              % new peak ky
    param.Dir(angle_num).phaOff =phase_irt;                                    % new phase offset
